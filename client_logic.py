...
def split_file(file_bytes, filename, version=1):
    ...
    for i in range(0, len(file_bytes), CHUNK_SIZE):
        ...
        # Calculate the SHA-256 hash of this specific chunk
        chunk_hash = hashlib.sha256(chunk_data).hexdigest()
        
        chunks.append({
            # ---> NEW: Embed the version number directly into the physical chunk name
            'chunk_name': f"{filename}_v{version}_part_{i}",
